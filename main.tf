...
resource "aws_iam_policy_attachment" "lambda_basic_policy_crypto_report" {
  name       = "lambda-basic-policy-attachment-crypto-report"
  roles      = [aws_iam_role.lambda_role_crypto_report.name]
  policy_arn = "arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole"
}

resource "aws_iam_policy_attachment" "lambda_sqs_full_access_crypto_report" {
